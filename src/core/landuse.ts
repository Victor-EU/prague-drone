// Ground classes painted into the land-use raster by tools/build-world.ts. The app turns them into
// the terrain's base colour. Colours are sRGB material bases before the grade (design.md §8.9).

export const Ground = {
  Urban: 0,
  Water: 1,
  Grass: 2,
  Wood: 3,
  Park: 4,
  Garden: 5,
  Orchard: 6,
  Cemetery: 7,
  Pitch: 8,
  Residential: 9,
  Industrial: 10,
  Road: 11,
  Cobbles: 12,
  Path: 13,
  Square: 14,
  Rail: 15,
  Meadow: 16,
  Scrub: 17,
  Rock: 18,
  Parking: 19,
  Vineyard: 20,
  Sand: 21,
  Construction: 22,
  Clay: 23,
  Farmland: 24,
} as const;
export type GroundClass = (typeof Ground)[keyof typeof Ground];

export const GROUND_COLOURS: Record<number, string> = {
  [Ground.Urban]: '#857d72',
  [Ground.Water]: '#3a4a52',
  [Ground.Grass]: '#7a8a52',
  [Ground.Wood]: '#44533b',
  [Ground.Park]: '#6b7d4c',
  [Ground.Garden]: '#5f7348',
  [Ground.Orchard]: '#7b8a50',
  [Ground.Cemetery]: '#56654a',
  [Ground.Pitch]: '#6f8a55',
  [Ground.Residential]: '#8b847a',
  [Ground.Industrial]: '#878073',
  [Ground.Road]: '#5d5a57',
  [Ground.Cobbles]: '#6b6660',
  [Ground.Path]: '#a39a88',
  [Ground.Square]: '#7a7468',
  [Ground.Rail]: '#5a5049',
  [Ground.Meadow]: '#8a9a5a',
  [Ground.Scrub]: '#5a6a45',
  [Ground.Rock]: '#8a8074',
  [Ground.Parking]: '#676460',
  [Ground.Vineyard]: '#6f7d4a',
  [Ground.Sand]: '#c2b28f',
  [Ground.Construction]: '#9a8d78',
  [Ground.Clay]: '#b0654a',
  [Ground.Farmland]: '#a0a06a',
};
