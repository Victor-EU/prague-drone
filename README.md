...
Refining the grade (design.md §12.3): `npm run compare -- --fit` also writes, for every viewpoint, the render before the LUT, its sky mask and the photograph at the render's size into `compare/fit/`; `node tools/lut-fit.ts` then fits the grade's parameters to the photographs' colour statistics and writes them with the LUT (`--report` prints the statistics for the current grade, `--detail` each frame's, `--masks` the sky masks it found). `node tools/sheet.ts` makes the final side-by-side sheet from the same captures, `compare/sheet/index.html`: every pair with the three questions to click pass or fail.

The motion and loading tests of design.md §12.2 and §10.2 run in headless Chrome on the GPU: `node tools/motion.ts route 1` (or `2`, fast mode) flies the route in real time and reports the hitches, `manual` flies five minutes by hand at random, `clouds` reseeds twenty times, `clock` slides the time of day from 04:30 to 23:00, `load` times the production build (`npm run build` first) over a fast connection, and `bench` measures the frame at every stop (`bench "" "quality=lite"` compares the presets; `bench "" "detail=0"` measures the close-up details of the facades, roofs, near trees and roses, which `?detail=0` turns off in development; `bench "" "fine=0"` measures the landmarks' fine tier of mouldings, tracery and balusters (design.md §7.1, M12), which `?fine=0` leaves out in development; `bench "" "mirror=0.4"` measures the river's mirror at another size (the full preset draws it at half the frame's since M13); `bench "" "relief=0"` measures the facades' relief (design.md §8.2, M14), which `?relief=0` leaves painted in development (`?relief=150` builds it within 150 m); `bench "" "world=world-m10"` compares against another world built into `public/world-m10`, which must be deleted before `npm run build`). They report this machine's numbers with whatever else is using its GPU.
