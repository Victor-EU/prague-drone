...
As built in M13, after the verdicts on the M12 sheet (§13 Progress). The **Dancing House** (`tools/landmarks/dancing-house.ts`) was rebuilt from the frames 8146 to 8173 and 9567 to 9581: Ginger is lofted from her section at eleven heights, a circle whose centre moves toward Fred as she narrows, so she keeps to his skin and above the waist leans over him; her skirt flares to 10 m across at its rim 6 m over the pavement, on ten tapered concrete blades that lean out from an oval of feet; the glass carries a mullion up every one of 32 vertices and a transom at every floor, with diagonals across the skirt's panes, in the detail and fine tiers; the platform between the two at the third floor stands on a strut, with its parapet; her top has a white lip and the small glass drum of the bar. Fred's windows sit in grey boxes half a metre proud of the plaster, the glass at their front, ten a row, the rows waving and none where Ginger leans on him or the body joins; wavy bands run round him between the rows; the Medusa is a denser tangle of tubes on a low drum. The body, lower than Fred by a storey, has the same boxes on its river and corner faces, bands between the rows and a coping. The model now also replaces a 15 m "building" of 7 m² that OSM has between the towers, and the legs mapped as 5 m parts, and claims the pavement before it, which the world build keeps clear of trees (7940 had one standing before Ginger). The **arch bridges** (`tools/landmarks/bridges.ts`) gained a classical cornice swept under the parapet along both faces, balusters on Mánes Bridge in place of its solid parapet (in the detail tier: 42,000 triangles, the one bridge with them), taller candelabra on Legion Bridge, and darker granite on Legion and Jirásek Bridges, which read pale and flat in 9486 and 8988.

Added in M14, the **Old Town Square set** (`tools/landmarks/old-town-square.ts`), the row 8607, 8608 and 8609 look at, modelled on their OSM footprints and parts and named nowhere on the screen: the **Kinský palace** (rococo, pale pink with white dressings: pilasters through the two upper storeys, an entablature, the two pavilions standing a little proud under segmental pediments, a balustraded attic with statues and urns, two portals with balconies over them, a mansard roof with dormers, the courtyard wings behind as plainer blocks); the **Stone Bell house** (Gothic sandstone ashlar, tall traceried windows on the square in two storeys over a pointed portal, string courses, the corner bell, the steep gabled roof with its dormers); and the **Týn school** (the arcade of four pointed arches on the square, three storeys of windows in surrounds, and the two Venetian gables, each a stack of semicircular arcs stepping up to a peak, the roof behind them). They replace their OSM buildings and parts; their neighbours stay the generic fronts with M14's relief, gables and arcades.
...
Built in M14 (§8.2's milestone), on the roofs and at the corners, placed at build time as props and made by the tile worker into the detail mesh (drawn to 1.6 km, since they break the silhouette): **gables** standing on the eave in the facade's plane on the old town's street fronts (a volute gable with its scrolls and a segmental cap, a stepped Renaissance gable, or a pedimented attic), on about a third of the eaves of 7 m or more, 3.5 to 8 m wide, 1.2 m thick with a coping on top, the dormers keeping to the eave's ends beside them; on the 19th-century blocks along the river a pediment over the middle of the front more often; **corner turrets** on the embankment blocks (a round bay from the first floor rising through the cornice under a bell cap and a finial, on a street corner of a house of five storeys or more, one in three); **bays** (oriels) on the blocks' fronts, one window wide, from the first floor to the cornice, 1.1 m proud, with the shader's windows on their fronts and a small roof; **attic figures**, a stone silhouette 2.2 m tall at the ends of the embankment fronts' cornices and an urn on the blocks' pediments (placed here, built with the relief of §8.2 within 300 m, being small); and the large dormers are **hipped boxes** now, not flat ones.
...
As built in M14, the facades in relief (§13, §14). The painted relief of M9 and M10 stays, and within 300 m of the camera the same parts stand out from the wall as **geometry**, which the sun lights and shadows as it does the landmarks: a cornice moulding (0.45 m proud, 0.5 m high) round every non-party wall under the eave, mitred at the corners, its top face the pale line the roofs' edges show from the air (8884); a string course over the ground floor; a plinth; the lesenes as strips; every window's surround as a frame 7 cm proud with its sill 14 cm proud, the hood over the first-floor windows of the rich fronts and the upper windows of the blocks (straight, segmental or triangular, as painted); balconies as a slab on consoles with an iron railing of balusters; portals as jambs, an arch ring or a lintel with its cornice, the keystone. Stucco (cartouches, ears, keystones, wreaths, shells), shutters and the shopfronts stay painted, being a few centimetres deep. The geometry is not shipped: the tile worker (`src/world/relief.ts`) builds it from the tile's footprints on request, in **chunks of 100 m** (a hundred to a tile), for the chunks within `reliefRange` of the camera (300 m on full, none on lite, §11), and the app drops a chunk 50 m beyond that; so a house's relief exists only while the drone is near it. A chunk within 140 m carries the full relief; farther out, to the range, it carries only what a pixel can still show at that distance (the cornices, string courses, sills, hoods, portals and balcony slabs, which cast the shadows that read), not the 7 cm surrounds, keystones and balusters, and is rebuilt at the other level when the camera crosses the line (with 20 m of hysteresis). The meshes arrive one a frame, so their upload does not pile into a hitch. The attic figures and urns of §8.1, 2 m tall, are built with the relief rather than shipped in the tiles' detail. To stand exactly where the shader paints, the worker computes the window grid, the storeys, the portal's column, the balconies and the hoods by the shader's own rules; the choices the shader had made from a floating-point hash of the building's seed (the trim's tone, the portal, the arches, the hood, the balcony, the door's wood, the shutters) are now made by an integer hash of the seed that JavaScript and GLSL compute bit for bit alike, so the two agree. The trim's tone is decided in the worker and carried in the flags byte. The painted railing is hidden where the geometry draws one. Relief walls, the bays' fronts and the arcades' piers take a surface of their own (`Surface.Trim`) that the street lamps light as they do the wall.
...
Built in M14, the **arcades** (loubí) of the squares as recesses: on Old Town Square's south row and its east side (the Týn school's and its neighbours'), on Malé náměstí's east and south sides and on the lower Malostranské náměstí's east row, every street-front edge of an Old Town or baroque house that stands within 4 m of the square's polygon and faces it gets the `Arcade` edge flag at build time, and the tile worker builds its ground floor as an arcade instead of a wall: piers at the window columns' boundaries, round arches between them, a soffit at the first floor and the back wall 3 m in, the fronts above starting at the first floor. The squares' rows themselves (§7.1) carry their own.
...
Changed in M13, the river of the detail programme (§14), against 8683, 8988, 8490, 8849, 9542, 9547, 8825 and 9486:

- **The waves** (`src/world/water.ts`): the water mesh carries a fourth attribute, metres to the nearest bank (a chamfer distance over the 5 m water grid, `tools/lib/river.ts`), the wind's fetch: the water lies glassy within a few metres of a wall and roughens out in the stream. The slopes are a sum: the current's ripples carried downstream at 9 m and 2.4 m, as before but weaker; the wind's chop across them, the wind from the west-south-west, in two tilings at 3.2 m and 4.9 m, the second turned 31°, so that seen at a grazing angle neither repeats as a grid (8988 showed the old tiling as a pattern); a swell of three sine trains at 6.5, 11 and 19 m along the wind, each moving at a deep-water wave's own speed under a slow envelope of its own, which sways the reflections without a band to see; and within 80 m the capillary ripples that give the sun its sparkle. The sampling point is warped slowly so the tilings never line up, and the small scales fade with distance, where their crests would share a pixel and alias. Far off, the roughness rises from 0.09 to 0.3, and the sun's glitter widens into a path.
- **The reflection's strength** rises toward a grazing angle: 37% of the reflection seen from above (8490, 8849, where the water's own colour shows), 88% seen edge on (8683, 8988, 9542, where the far river carries the sky and the far bank almost whole), on a curve softer than Fresnel's, as the ripples' facets spread the angles; M8's flat 55% is gone. Where the mirror shows the city it takes 78% of the mix by day and 90% at night (70% before), and under overcast the water lies a shade darker than the sky it carries. The reflections are drawn out into columns by facets tilting ±0.03 rad by day and ±0.09 at night (0.012 and 0.03 before), with a flatter kernel: the lamps' streaks run down the water as in 9542 and 9547. The mirror is drawn at half the frame's size on the full preset (40% before); `?mirror=0.4` measures it.
- **The quays** (`tools/lib/river.ts`): the walls are the dark grey-brown of the photographs' stone (they were pale sand); a flight of stone steps runs down the face to the water every 150 m or so of wall in the core; along the Rašín embankment the retaining wall behind the lower quay opens in round arches every ten metres, the Náplavka's vaults, as dark openings (`tools/lib/walls.ts`).
- **The weirs' walkways** (8825): along the crest of every weir over 60 m in the seen world, a timber walkway: a plank deck a metre over the upper water, a pair of piles every 2 m on the downstream side with their heads above the deck and a tie beam along them, a lower row upstream, bearers under the deck; run on past the mapped crest's ends to the banks, since the photograph stands at its start. Five of them, 42,000 triangles in the detail tier with the stairs.
...
Changed in M13 (`src/life/river.ts`), against 8849 ("the boat needs finetuning") and 9486 ("the boats are simplistic"): the tour boats have hulls that flare from the waterline to the deck over a navy or dark boot-top, a rubbing strake, a bulwark, a saloon with its windows as panels in a row, a railing of posts and rails round the open deck, benches, a canopy on posts over the after half, the wheelhouse forward, a mast and the flag on the stern; the steamer its tall black funnel with a red band; the restaurant boat a long glass saloon with mullions and a small wheelhouse. The pedal boats have a windscreen and two striped bucket seats, orange and cream, with their riders. Six kayaks meander on the same water as the pedal boats, quicker, in the colours of hire kayaks, each with its paddler and paddle.

Changed in M14 (`src/life/trams.ts`), against 8942 ("the tram needs more details"): the Tatra T3 has its rounded ends with the three-piece windscreen and headlight, the cream band and the cream roof with its vents, the red band with the three folding doors in it, the window band with its pillars and the sliding lights, the skirt, the bogies' wheels and the pantograph on its frame; the 15T its long window band with the doors and the black pillars, the raked cab glass, the roof's equipment boxes and the bellows.
...
| Reflections | Planar reflection for the river. As built in M4: at 40% resolution, every other frame, within 2.2 km, the sky and clouds computed by the water shader (§8.5); at half the frame's size on the full preset since M13 | The evening frames depend on it |
...
| Facades' relief as geometry (M14) | within 300 m, in full within 140 m | none (painted) |
...
Changed in M13: 8825 moved 65 m north-west onto the east end of the Old Town weir's crest, 4 m over the water, heading 300, with the photograph's skyline (the Castle, Charles Bridge, the museum's gable at the right) and the weir's foam running across the frame; at its M8 place it stood on the quay south of the Smetana Museum. The timber pier the photograph stands on runs from Novotného lávka toward the crest at a bearing OSM's weir line does not have (the mapped crest ends 60° from it), so the walkway lies under the camera, out of the frame; the crest walkways themselves are §8.5's.
...
| M13 | The river (§8.5, §8.8, §8.4): the ripples' tiling fixed and a wave model with wind and fetch; the reflection's strength rising toward a grazing angle, the mirror sharper near, longer streaks at night; sun glitter; the quays rebuilt from the photographs (dark rusticated walls, coping, railings, stairs, Náplavka's vaults); the Old Town weir's pile row and the lávka; the boats with their real look (tour boats with window rows, railings, wheelhouse and funnel; pedal boats with their seats and canopy; kayaks); the arch bridges' faces (cornices, arch rings, balustrades, tall lamps, cutwater caps); the Dancing House rebuilt (added after the M12 verdicts, §13 Progress); the water's colour re-fitted per light family. Willows moved to M16, where every species' silhouette is built: doing one twice is waste | 8683, 8694, 8849, 8988, 8825, 9547, 9542, 8490, 9486, 8158 and 9204 show water that reflects the city and the sky as the photographs' does, banks of dark stone, bridges with their rings and lamps, a river full of boats, and the Dancing House as it stands; no pair gets worse; within budget |
...
| M16 | Trees (§8.4): species silhouettes (irregular, layered limes and chestnuts, poplars, willows on the banks and islands, moved here from M13), branch structure near, canopies that read as a mass with texture, the orchards' gnarled trees | 8725, 8809, 8753, 8385, 8884 and 8490 show trees, not balls or confetti; no pair gets worse; within budget |
...
- **The verdicts** on the sheet were given by the user on 2026-09-26, below.

**The user's verdicts on the M12 sheet, 2026-09-26.** Every pair was judged on its note rather than the three buttons (8385 alone passes all three, "the buildings don't look right comparing to the photo"). The notes, and where each goes:

- **The Dancing House** (8158 "needs major rework, the dancing house", "looks absolutely horrible"; 7940 "the dancing house is wrong, there is no tree in front"): rebuilt first, in M13, since it stands on the Rašín embankment and 8158 is the loudest verdict. The M12 model had Ginger as a plain swelling glass column apart from Fred, without her flared skirt on its slanting legs, her mullions or the platform between the two; Fred's windows flat in even rows, without their projecting boxes and the wavy bands; the body's windows missing on the faces the camera sees.
- **The river** (8683 "needs some small waves"; 8490 "put some small waves on the river, animate it"; 8988 "waves don't look natural"; 9547, 9542 "missing reflections on the river"): M13's wave model, the reflection's strength at a grazing angle, and the night streaks.
- **The boats** (8849 "the boat needs finetuning"; 9486 "the boats are simplistic"): M13.
- **The bridges** (9486 "bridges don't look right"): the arch bridges' faces, in M13, since they stand in the river's frames.
- **The light** (9204 "the light doesn't seem right"): the Čertovka under overcast, flat and bright in the render where the photograph is dark under the trees with the water carrying the sky; checked with the water in M13, the rest to M17.
- **The tram** (8942 "the tram needs more details"): M14, with the squares.
- **The houses** (7924, 7944, 8753, 8372, 8440, 8725, 8722 "add more details to the buildings", "missing details"; 8607, 8608, 8903, 8942, 8809, 8082, 8777, 8884, 8825, 8694, 8988, 8704 "missing details, also the architecture doesn't look right", 8704 "missing too much details comparing to the photo on the building"): M14, the facades in relief and the squares' rows, and M15, the spires and the Castle. Twenty of the thirty pairs carry this note: the facades are the largest gap left.

**M13, built 2026-09-26.** The river, and the Dancing House first (§14):

- **The Dancing House** (§7.1) rebuilt: Ginger's flared skirt on its slanting legs, her waist and swell leaning over Fred, mullions and transoms, the platform between them; Fred's windows in their projecting boxes in waving rows between wavy bands, the Medusa; the body's boxes and coping; a stray OSM building between the towers replaced, the pavement kept clear of trees (8158, 7940).
- **The water** (§8.5): the fetch attribute, the wind's chop in two tilings, the swell, the sparkle, the small scales fading with distance; the reflection's strength rising to a grazing angle, longer streaks by day and night, the mirror at half size (8683, 8988, 8490, 8849, 9542, 9547).
- **The quays and the weirs** (§8.5): dark stone walls, stairs to the water, the Náplavka's vaults, timber walkways along five weirs (8825).
- **The boats** (§8.8): hulls, window rows, railings, canopies, wheelhouses, funnel, flags; the pedal boats' striped seats; kayaks (8849, 9486).
- **The bridges** (§7.1): cornices, Mánes Bridge's balusters, taller candelabra, darker granite (9486, 8988).
- **Viewpoint** 8825 moved onto the walkway (§12.1).

Measured with the synced benchmark at 2048 × 1536 in three alternating rounds against M12's world, built into `public/world-m12` and loaded with `?world=world-m12`, both under M13's code: 17.4 against 16.5 ms a frame (17.5, 17.5 and 17.3 against 16.2, 17.0 and 16.3), the mean of each stop's median: the new content (the walkways' piles, Mánes Bridge's balusters, the boats, the Dancing House) costs about 0.9 ms, mostly on the low pass over the river and the Old Town orbit. The mirror at half size against 40% (`bench "" "mirror=0.4"`): 17.4 against 17.0 ms (17.8, 16.9 and 17.6 against 16.8, 17.3 and 16.9), about 0.4 ms, within the rounds' spread. Against M12's 16.4 ms the milestone costs about 1 ms in all, well inside the 25 ms budget (§11). Another application kept the GPU about 88% busy during the runs, as during M12's. The world is 25.9 MB (`landmarks.bin` 6.7 MB, up 1.4 MB for the walkways, the balusters and the Dancing House); the production build 780 kB of JavaScript.

Known gaps after M13:

- **9542, 9547**: the floodlit fronts are white and bright where the photographs' are sodium orange and dim, and their streaks broad where the photographs' are the points of lamps; the night's light is M17's.
- **9204**: the Čertovka under overcast is lit flat and bright; the light family, M17. The mill's wing and the houses behind it stay plain (M14).
- **8988, 9486**: the bridges are still lighter and flatter than the photographs' granite in the evening light; the facades along the embankments are M14's.
- **8825**: the pier the photograph stands on is not in the frame (§12.1); the weir's walkway is seen from the drone and from the quays, not from this camera.
- **The Náplavka's vaults** are dark plates in the retaining wall; the lower quay's life (tables, the moored boats' gangways) is not built.
- **Willows** wait for M16, with every species.
- **The verdicts** on the sheet after M13 are the user's.

---

**M14, built 2026-09-26.** The facades in relief, the milestone twenty of the thirty verdicts pointed at (§13 Progress):

- **Relief as geometry** (§8.2): cornices, string courses, plinths, lesenes, surrounds, sills, hoods, balconies with their railings and portals, built by the tile workers on the shader's own grid and choices (an integer hash shared by GLSL and JavaScript, `core/buildings.ts`), streamed in chunks of 100 m within 300 m of the camera at two levels, dropped as the camera leaves (8777, 8082, 8884, 8158).
- **Gables, turrets, bays, figures** (§8.1): volute, stepped and pedimented gables on a third of the old town's street eaves and pediments over the embankment blocks; corner turrets under bell caps and bays one window wide on the blocks by the river; stone figures at the ends of their cornices and urns on the pediments; the large dormers hipped (8988, 8884, 8809).
- **The squares' arcades** (§8.3): 33 fronts on Old Town Square, Malé náměstí and the lower Malostranské náměstí with their ground floors taken back behind piers and round arches (8607, 8608).
- **The Old Town Square set** (§7.1): the Kinský palace, the Stone Bell house and the Týn school modelled on their footprints (`tools/landmarks/old-town-square.ts`), with the landmarks' windows dressed in surrounds by the shader's grid (8607, 8608, 8609).
- **The trams** (§8.8): the T3 and the 15T with their doors, window pillars, lamps, wheels and roof gear (8942).

Measured with the synced benchmark at 2048 × 1536 in two alternating rounds, the benchmark now waiting at each stop for the relief to stream in before it times the frame (it had timed the stops before their chunks arrived, and so measured the relief at nothing): M14 20.4 and 21.4 ms a frame (the second with a 40 ms first stop, the first uploads) against M13's world 19.4 and 19.6 (`?world=world-m13`), and against M14 with the relief left painted (`?relief=0`) 20.1 and 19.9: the relief costs about half a millisecond to a millisecond on the route, the props about half a millisecond, the milestone 1 to 2 ms in all, well inside the 25 ms budget (§11). A first version at 200 m chunks with the full relief to 300 m had cost 11 ms at the low pass over Malá Strana (1.3 million triangles resident) and hitched by 100 ms as chunks of 300,000 vertices uploaded together: hence the two levels, the smaller chunks and one arrival a frame. Another application kept the GPU busy during the runs, as during M12's and M13's. The world is 26.2 MB (`landmarks.bin` 6.9 MB, the tiles 6.6 MB; the relief ships nothing); the production build 785 kB of JavaScript, up 5 kB for the relief and the streaming.

Known gaps after M14:

- **At the hero frames' distances** the relief reads as shadow lines under sills and cornices and as the breakup of a front's silhouette by gables; it is within 50 m (8777, 8082) that the surrounds and hoods show as what they are. The windows themselves stay painted: the glass is not recessed, so a front seen at a grazing angle shows no reveals.
- **8608**: the Kinský palace's front is pale in the render where the photograph's is lit by the low sun; the row's colours are approximate. The people on the square are still the boxes of M6.
- **8942**: the tram has its parts but is a small thing in the frame; the left foreground building is an OSM part drawn flat (a landmark's stand-in), unchanged.
- **8988, 9486**: the embankment fronts carry gables, bays and turrets now, but the photographs' Art Nouveau fronts are richer than the block style's grid; the bridges' tone is unchanged (M17's light).
- **8809, 7924, 7940, 7944, 8753, 8385, 8372**: the far skyline and the Castle wait for M15; the trees for M16.
- A camera placed inside an OSM outline that has no courtyard hole (the Thun palace on Nerudova) sees the far walls' relief through the culled back faces; the drone never stands there.
- **The verdicts** on the sheet after M14 are the user's.
...
Decided on 2026-09-26 by the user, the verdicts on the M12 sheet (§13 Progress):

- **The Dancing House is rebuilt first**, in M13, before the river: "needs major rework", "looks absolutely horrible" (8158), and from Petřín (7940) it is wrong and hidden behind a tree that is not there. The tram's detail goes with the squares in M14; the houses' notes on twenty pairs stand as the brief for M14 and M15.
